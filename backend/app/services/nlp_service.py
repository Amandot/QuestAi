...
class NLPService:
    ...
    def extract_entities(self, text: str) -> Dict[str, List[str]]:
        ...
        for entity_type, entities in entities_by_type.items():
            ...
            if len(entity_list) >= 2:  # Need at least 2 entities of same type for distractors
    ...
    def generate_question_answer(self, context: str) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            question = self.t5_tokenizer.decode(question_outputs[0], skip_special_tokens=True)
            
            # Generate answer using a different prompt
            answer_prompt = f"Answer this question based on the text: {question} Text: {context}"
            answer_inputs = self.t5_tokenizer.encode(answer_prompt, return_tensors="pt", max_length=512, truncation=True)
            ...
            answer = self.t5_tokenizer.decode(answer_outputs[0], skip_special_tokens=True)
    ...
    def verify_answer_in_text(self, answer: str, text: str) -> bool:
        ...
        return clean_answer in clean_text or any(
            clean_answer in sentence.strip() 
    ...
    def get_distractors(self, answer: str, entity_type: Optional[str], entities_by_type: Dict[str, List[str]], text: str) -> List[str]:
        ...
        if entity_type and entity_type in entities_by_type:
            same_type_entities = [
                entity for entity in entities_by_type[entity_type] 
        ...
        if len(distractors) < 3:
            ...
            for related_type in related_types:
                if related_type in entities_by_type and len(distractors) < 3:
                    available = [
                        entity for entity in entities_by_type[related_type]
                        if entity.strip().lower() != answer_clean and entity not in distractors
                    ]
                    needed = 3 - len(distractors)
                    distractors.extend(random.sample(available, min(needed, len(available))))
    ...
    def _get_generic_distractors(self, entity_type: Optional[str], answer: str) -> List[str]:
    ...
    def analyze_text_complexity(self, text: str) -> Dict:
        ...
        return {
            ...
            "suggested_difficulty": "Hard" if complexity_score > 0.7 else "Medium" if complexity_score > 0.4 else "Easy"
        }
    
    def generate_short_answer_question(self, context: str, page_num: int) -> Optional[Dict]:
    ...
    def _determine_bloom_level(self, question: str) -> str:
        ...
        if any(word in question_lower for word in ["analyze", "compare", "contrast", "examine", "why", "how"]):
            return "Analyze"
        elif any(word in question_lower for word in ["apply", "use", "implement", "solve", "demonstrate"]):
            return "Apply"
        elif any(word in question_lower for word in ["explain", "describe", "interpret", "summarize", "what"]):
            return "Understand"
        else:
            return "Remember"
    ...
    def _determine_difficulty(self, context: str, entity_type: Optional[str] = None) -> str:
        ...
        complex_sentences = sum(1 for sent in doc.sents if len(list(sent.noun_chunks)) > 2)
        ...
        complexity_ratio = complex_sentences / total_sentences if total_sentences > 0 else 0
    ...
    def _modify_sentence_for_false(self, sentence: str) -> str:
        ...
        for token in doc:
            if token.like_num:
                ...
                new_num = str(int(token.text) + random.randint(1, 10)) if token.text.isdigit() else token.text
